...
user.use(
  ...
  user.get('/list', verifyAuthToken, async (request, response) => {
    try {
      ...
      return response
      .status(400)
      .send(`Could not get users. Error: ${(error as Error).message}`);
    }
  });
  ...
  user.get('/:id', verifyAuthToken, async (request, response) => {
    ...
    try {
      ...
      return response
      .status(400)
      .send(`Could not get a user. Error: ${(error as Error).message}`);
    }
  });
  ...
  user.post('/', verifyAuthToken, async (request, response) => {
    ...
    try {
      ...
      .status(400)
      .send(
        `Could not create a user ${u.username} ${u.email}. Error: ${
    ...
    user.put('/', verifyAuthToken, async (request, response) => {
      ...
      user.delete('/:id', verifyAuthToken, async (request, response) => {
        ...
        try {
          ...
          .status(400)
          .send(
            `Could not delete a user ${id}. Error: ${(error as Error).message}`
            );
          }
        });
        ...
        user.post('/login', (request, response) => {
          ...
          .catch((error) => {
            return response
            .status(400)
            .send(`Login Failure. Error: ${(error as Error).message}`);
          });
        });
